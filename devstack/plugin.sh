...
DEVICEDEMO_PLUGIN=$DEST/devicedemo/devstack
source $DEVICEDEMO_PLUGIN/devicedemo.sh

if [[ "$1" == "stack" && "$2" == "install" ]]; then
    # This phase is executed after the projects have been installed
    echo "Devicedemo plugin - Install phase"
    if is_service_enabled devicedemo; then
        install_devicedemo
    fi
    ...
    echo "Devicedemo plugin - Post-config phase"
    if is_service_enabled devicedemo; then
    ...
    echo "Devicedemo plugin - Extra phase"
    if is_service_enabled devicedemo; then
        init_devicedemo
    fi
elif [[ "$1" == "stack" && "$2" == "test-config" ]]; then
    # This phase is executed after Tempest was configured
    echo "Devicedemo plugin - Test-config phase"
    if is_service_enabled devicedemo; then
